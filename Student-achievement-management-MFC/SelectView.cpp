...
void CSelectView::OnInitialUpdate()
{
	...
	m_treeCtrl->InsertItem(TEXT("添加成绩"), 0, 0, NULL);
	m_treeCtrl->InsertItem(TEXT("修改成绩"), 0, 0, NULL);
	m_treeCtrl->InsertItem(TEXT("删除成绩"), 0, 0, NULL);
	m_treeCtrl->InsertItem(TEXT("显示全部成绩"), 0, 0, NULL);
...
void CSelectView::OnTvnSelchanged(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	else if (str == TEXT("添加成绩"))
	{
		::PostMessage(AfxGetMainWnd()->GetSafeHwnd(), NM_B, (WPARAM)NM_B, (LPARAM)0);
	}
	else if (str == TEXT("修改成绩"))
	{
		::PostMessage(AfxGetMainWnd()->GetSafeHwnd(), NM_C, (WPARAM)NM_C, (LPARAM)0);
	}
	else if (str == TEXT("删除成绩"))
	{
		::PostMessage(AfxGetMainWnd()->GetSafeHwnd(), NM_D, (WPARAM)NM_D, (LPARAM)0);
	}
	else if (str == TEXT("显示全部成绩"))
	{
